...
const (
	...
	testNodeID   = "node-1"
	testPVCNs    = "default"
	// volumeID differs from the PV name on purpose: for a static PV the two are
	// unrelated, and only the path carries the PV name. Verified against kubelet:
	// the directory segment is the PV name.
...
// A mount published through NodePublishVolume is already recorded, so the
// recovery path must not reach the API server for it.
...
// The csi-node restart case: the labels have to be rebuilt from the mount path.
func TestEnsureVolumeInfo_PopulatesOnMiss(t *testing.T) {
	d, cs := newVolumeInfoTestService(testPV(), testPod())

	d.ensureVolumeInfo(context.Background(), testVolumeID, testVolumePath, testPodUID)

	// The juicefs_ prefix and node_name label are added by the wrapped Registerer,
	// so collecting the vec directly yields the bare name.
	expected := `
...
`
	...
	// A PVC get would be redundant: claimRef already carries the PVC name.
...
// A pod can only mount a PVC from its own namespace, and config.NodeName is
// left empty for --by-process=true deployments, so the list has to be scoped by
// the claimRef namespace and the driver's own node id.
...
// Every failure leaves the mount unmarked so a later stats call can retry, and
// never emits a partially filled series.
...
// --by-process=true builds the driver without a Kubernetes client, so recovery
// has to be a no-op rather than a nil dereference.
